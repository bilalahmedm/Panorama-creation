...
def plot_matches(img1: np.ndarray, img2: np.ndarray, kp1: List[cv2.KeyPoint], kp2: List[cv2.KeyPoint], matches: List[cv2.DMatch],inliers=None, group_no: str = None, name: str = None) -> None:
    """
    ...
    result = cv2.drawMatchesKnn(img1, kp1, img2, kp2, [[m] for m in matches], None, **draw_params)
    ax1.imshow(result)
